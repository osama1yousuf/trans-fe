          ...
          <div className="bg-white rounded shadow-lg py-4 px-6">
            <nav className="flex justify-between">
              ...
              <div  className="relative">
                <button onClick={profileSetting} className="relative inline-flex mt-1 justify-end  cursor-pointer ">
                 <BsPersonCircle className="text-gray-800" size={40} /> 
                </button>

               {profileModal && <div className="right-0 mt-4 w-48 absolute bg-gray-100 rounded shadow-lg z-50">
                  <ul className="py-2">
                    <li onClick={()=>{ setProfileModal(false)
                       router.push('/setting')}} className="block px-3 py-1 text-gray-900 cursor-pointer hover:bg-gray-200">Settings</li>
                    <li onClick={()=>{
                      localStorage.clear()
                      setProfileModal(false)
                      router.push('/')
                    }} className="block px-3 py-1 text-gray-900 cursor-pointer hover:bg-gray-200">Logout</li>
                  </ul>
                </div>}
              </div>
        ...
        >
          <div className="h-full px-3 py-4 overflow-y-auto bg-white">
            <ul className="space-y-2 pt-4 font-medium">
              {openTabs && openTabs.map((val, index) => {
                return (
                  <li>
                    {console.log("DANCEEE", !val.Content && val.name !== "Location" ? `/${val.url}` : '')}
                    <Link
